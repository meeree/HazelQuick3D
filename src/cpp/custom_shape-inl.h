...
{
    Bind(m_vao);
    if(m_indices->size() > 0) // Use index buffer.
    {
        Bind(m_ind_buffer);
        gl::DrawElements(m_prim_type, m_draw_count, gl::IndexType::kUnsignedInt);
        Unbind(m_ind_buffer);
    }
    else // Just draw vertices.
    {
        Bind(m_buffer);
        gl::DrawArrays(m_prim_type, m_draw_start, m_draw_count);
        Unbind(m_buffer);
    }
    Unbind(m_vao);
}
